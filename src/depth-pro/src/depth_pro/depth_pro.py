...
class DepthPro(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, Optional[torch.Tensor]]:
        ...
        encodings = self.encoder(x) #620ms
        features, features_0 = self.decoder(encodings) #192ms
        canonical_inverse_depth = self.head(features)#42ms
    ...
    def infer(
        ...
        #TO HERE IS 250US ROUGHLY

        #with torch.autograd.profiler.profile(use_cuda=True) as prof:
        canonical_inverse_depth, fov_deg = self.forward(x) #take 800ms, bruh.
        #print(prof.key_averages().table(sort_by="cuda_time_total"))
